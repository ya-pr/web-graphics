...
d3.json("../../../../data/world-110m.json", function(error, world) {
    ...
    d3.json("data/russia.json", function (error, russia) {
        ...
        var max_weight = d3.max(russia.map(
            function (d) {
                return d.weight;
            }
        ));
        size.domain([0, max_weight]);
        draw(russia);
...
function click() {
    ...
    var html = '<p class="title">' + d.name + '</p>' +
        '<p class="weight"><span>' + format(d.weight) + '%</span> of total queries for tourist destinations</p>';
    if (d.children) {
        html = html + '<p class="top_name">The most frequent searches for this destination</p>' +
            '<p class="note">Share of queries for the selected destination, %</p>' +
...
function add_top_country(data) {
    info.html('');
    var html = '<p class="title">5 Most Popular Countries in the Americas</p>' +
        '<p class="note">Average share of all tourism destination queries, %</p>' +
